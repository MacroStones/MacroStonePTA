...
static void rbtree_print(RBTree tree,struct jiazu *now)
{
    if(tree != NULL)
    {
    	now->next=(struct jiazu *)malloc(sizeof(struct jiazu));
    	now->next->direction='L';
    	now->next->deep=now->deep +1;
    	now->next->next=NULL;
        rbtree_print(tree->left,now->next);
        struct jiazu *tmp;
        tmp=head;
        for(int i=0;i<now->deep;i++){
            if(tmp->next==NULL)
                break;
            if(tmp->direction != tmp->next->direction)
                printf("   ║\t");
            else
                printf("\t");
            tmp=tmp->next;
        ...
        now->next->direction='R';
        rbtree_print(tree->right,now->next);
        free(now->next);
    }
}
...
void print_rbtree(RBRoot *root) 
{
	head=(struct jiazu *)malloc(sizeof(struct jiazu));
	head->direction='R';
	head->deep=0;
	head->next=NULL;
    if (root)
        rbtree_print(root->node,head);
}
...
int main(){
	...
	1.将结点插入到红黑树中\n\
	2.删除结点(key为节点的值)\n\
	3.(递归实现)查找红黑树中键值为key的节点\n\
	4.(非递归实现)查找红黑树中键值为key的节点\n\
	5.返回最小结点的值\n\
	6.返回最大结点的值\n\
	7.前序遍历红黑树\n\
	8.中序遍历红黑树\n\
	...
	switch(n){
		...
		case 2:
			scanf("%d",&tmp);
			delete_rbtree(tree,tmp);
			break;
		case 3:
			scanf("%d",&tmp);
			printf("%d",rbtree_search(tree,tmp));
			break;
		case 4:
			scanf("%d",&tmp);
			printf("%d",iterative_rbtree_search(tree,tmp));
			break;
		case 5:
			rbtree_minimum(tree,&tmp);
			printf("%d",tmp);
			break;
		case 6:
			rbtree_maximum(tree,&tmp);
			printf("%d",tmp);
			break;
		case 7:
			preorder_rbtree(tree);
			printf("\n");
			break;
		case 8:
			inorder_rbtree(tree);
